...
events over a TCP socket using the shared protocol format.

Usage:
    python webcam_to_dvs.py [--host HOST] [--port PORT] [--threshold THRESH]
                            [--width W] [--height H] [--fps FPS]
                            [--source SOURCE] [--visualize]
"""
...
class DVSEmulator:
    ...
    def process_frame(self, gray_frame: np.ndarray) -> list[tuple[int, int, int, int]]:
        ...
        on_mask  = diff >  self.threshold   # brightness increased
        off_mask = diff < -self.threshold   # brightness decreased
...
def pack_frame_packet(frame_id: int, events: list[tuple[int, int, int, int]]) -> bytes:
    """Serialize a frame's events into the binary wire protocol."""
...
class TCPSender:
    """Manages TCP connection to the C++ receiver."""
    ...
    def connect(self, timeout: float = 30.0):
        """Attempt to connect, retrying until timeout."""
    ...
    def send_packet(self, data: bytes):
        ...
        # Prefix with 4-byte length so receiver knows how much to read
...
def run_with_webcam(args):
    """Main loop using OpenCV webcam capture."""
    try:
        import cv2
    except ImportError:
        print("ERROR: OpenCV not found. Install with: pip install opencv-python")
        print("       Falling back to synthetic mode. Use --source synthetic")
        sys.exit(1)
    ...
    print(f"[sender] Streaming {args.width}x{args.height} @ {args.fps} FPS (threshold={args.threshold})")
    ...
    try:
        while True:
            ...
            if frame_id % 30 == 0:
                print(f"  frame={frame_id:06d}  events={len(events):5d}  packet={len(packet)} bytes")
...
def run_with_synthetic(args):
    """Synthetic test pattern — no webcam needed. Simulates a moving blob."""
    emulator = DVSEmulator(args.width, args.height, args.threshold)
    sender = TCPSender(args.host, args.port)
    sender.connect()
    ...
    try:
        while True:
            ...
            # Generate synthetic grayscale frame with a moving bright circle
            ...
            # Also add a second smaller blob moving opposite direction
...
def main():
    ...
    if args.source == "webcam":
        run_with_webcam(args)
    else:
        run_with_synthetic(args)
